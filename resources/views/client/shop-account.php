...
<div class="col-lg-12">
    <div class="card">
        <div class="card-header card-product d-flex justify-content-between" id="<?= $category['id']; ?>" style="background-image: linear-gradient(to right, <?=$CMSNT->site('theme_color');?>, <?=$CMSNT->site('theme_color');?>, <?=$CMSNT->site('theme_color2');?>);">
            <div class="header-title">
                <h5 class="card-title" style="color:white;"><img src="<?=base_url($category['image']);?>" width="30px"
                        class="mr-2" /><?=$category['name'];?></h5>
            </div>
        ...
        <div class="card-body">
            <div class="row">
            <?php foreach ($CMSNT->get_list("SELECT * FROM `products` WHERE `status` = 1 AND `category_id` = '" . $category['id'] . "' ORDER BY `stt` ASC ") as $product) : $conlai = $product['id_api'] != 0 ? $product['api_stock'] : $CMSNT->get_row("SELECT COUNT(id) FROM `accounts` WHERE `product_id` = '".$product['id']."' AND `buyer` IS NULL AND `status` = 'LIVE' ")['COUNT(id)'];
                if($CMSNT->site('hide_product_empty') == 1){
                    if($conlai == 0){
                        continue;
                    }
                                    ...
                                    <tbody>
                                        <tr>
                                            <td class="text-center" style="width: 20%;">
                                                <div class="fs-3 fw-semibold" style="background: <?=$CMSNT->site('theme_color');?>;color: white;font-size:5px; border-radius: 100%;width: 4rem;height: 4rem;display: flex;-webkit-box-align: center;align-items: center;-webkit-box-pack: center;justify-content: center;text-align: center;margin: 0 auto;"><?=$conlai > 0 ? format_cash($conlai) : '<img style="width: 120%;" src="'.base_url().'resources/images/sold.png">' ?></div>
                                            </td>
                                            <td class="fs-sm text-muted">
                                               
                                                <div class="text-muted my-1" style="font-size:17px;"><?= __($product['name']); ?></div>
                                                <div class="text-muted my-1"><span class="d-block font-size-sm text-muted">
                                                        <strong style="font-size:11px" class="text-secondary"><del><?= format_currency($product['price'] + $product['price'] * 10 / 100); ?></del></strong> <strong class="text-danger" style="font-size:14px">» <?= format_currency($product['price']); ?></strong></span>
                                                </div>
                                                <div class="fs-sm">
                                                    <a class="btn btn-sm btn-outline-secondary" href="javascript:void(0)" onclick="info(`<?= $product['content']; ?>`)">
                                                        <i class="fa fa-circle-info opacity-50 me-1"></i> Thông tin</a>
                                                    <a class="btn btn-sm btn-outline-primary" href="javascript:void(0)" onclick="modalBuy(<?= $product['id']; ?>, <?= $product['price']; ?>, `<?= __($product['name']); ?>`)">
